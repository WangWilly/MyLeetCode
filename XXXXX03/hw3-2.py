...
def process_queries(queries: list[list[str]]) -> list[str]:
    ...
    for q in queries:
        ...
        if action == "SET":
        ...
        elif action == "COMP_AND_SET_AT":
            key, field = data[0], data[1]
            timestamp = int(data[2])
            expected_val, new_val = data[3], data[4]
            if key not in ttl_state:
                res.append("")
                continue
            if field not in ttl_state[key]:
            ...
            if not ttl_state[key][field].isOk(timestamp):
                res.append("")
                continue
            if ttl_state[key][field].val != expected_val:
                res.append("")
                continue
            ttl_state[key][field].val = new_val

        elif action == "COMP_AND_DEL_AT":
            key, field = data[0], data[1]
            timestamp = int(data[2])
            expected_val = data[3]
            if key not in ttl_state:
